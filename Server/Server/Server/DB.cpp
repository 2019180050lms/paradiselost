...
void DB::get_user_data(wchar_t* id, wchar_t* name, float* x, float* y, float* z, int* exp, int* level, int* weapon, int* type, int* stage)
{
    SQLCHAR user_name[NAME_LEN];
    SQLINTEGER user_exp = 0, user_level = 0, user_weapon = 0, user_type = -1, user_stage = 0;
    SQLFLOAT user_x = 0, user_y = 0, user_z = 0;
    SQLLEN cbX = 0, cbY = 0, cbZ = 0, cbName = 0, cbExp = 0, cbLevel = 0, cbWeapon = 0, cbType = 0, cbStage = 0;
    ...
    if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO) {
        ...
        retcode = SQLBindCol(hstmt, 9, SQL_INTEGER, &user_stage, 12, &cbStage);
        ...
        for (int i = 0; ; i++) {
            ...
            if (retcode == SQL_ERROR)
                ...
                std::cout << user_name << ", " << user_x << ", " << user_y << ", "
                    ...
                    << user_weapon << ", " << user_type << ", " << user_stage << std::endl;
            }
            else
                break;
        }
...
void DB::add_user_data(wchar_t* id, wchar_t* name, float* x, float* y, float* z, int* exp, int* level, int* weapon, int* type, int* stage)
{
    retcode = SQLAllocHandle(SQL_HANDLE_STMT, hdbc, &hstmt);
    wchar_t add_user[200] = L"";
    if (id[0] == '\0')
        return;
    swprintf_s(add_user, L"EXEC add_user_data %s, %s, %f, %f, %f, %d, %d, %d, %d, %d", id, name, *x, *y, *z, *exp, *level, *weapon, *type, *stage);
...
void DB::update_user_data(wchar_t* id, wchar_t* name, float* x, float* y, float* z, int* exp, int* level, int* weapon, int* stage)
{
    retcode = SQLAllocHandle(SQL_HANDLE_STMT, hdbc, &hstmt);
    wchar_t update[200] = L"";
    if (id[0] == '\0')
        return;
    swprintf_s(update, L"EXEC update_user %s, %s, %f, %f, %f, %d, %d, %d, %d", id, name, *x, *y, *z, *exp, *level, *weapon, *stage);
